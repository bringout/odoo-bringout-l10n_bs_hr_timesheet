...
class ProjectTaskTimeSheetBs(models.Model):
    ...
    timesheet_new_ids = fields.One2many(
        ...
        domain = [
           ('write_date', '>', fields.Datetime.now() - timedelta(days=3) ),
           ('create_date', '>', fields.Datetime.now() - timedelta(days=3) )
           #('in_payroll','=',False) previše usporava upit 
        ],
        string='Šihtarice unesene posljednjih 2 dana')

    ##ovo ne može jer su onda nove šihtarice readonly
    #timesheet_new_ids = fields.One2many(
    #    comodel_name='account.analytic.line', 
    #    compute="_compute_new_timesheet_ids", 
    #    inverse_name='task_id',
    #    string='Šihtarice unesene posljednjih 7 dana',
    #    readonly=False
    #)
    #@api.depends("timesheet_ids")
    #def _compute_new_timesheet_ids(self):
    ...
    #                #('date', '>', task.default_date - timedelta(days=8) )
    #                #('in_payroll','=', False) 
    #                ]) 
    #                #order="date desc")
    #        task.timesheet_new_ids = lines
